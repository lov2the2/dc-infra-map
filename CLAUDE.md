...
- `components/floor-plan/` — Floor plan visualization (floor-plan-grid, rack-card, floor-plan-canvas with useMemo-based derived state instead of useEffect for performance; unplaced racks render with 50% opacity + dashed border + label; placed racks render with solid style); floor space management (floor-space-manager with "Unplaced Racks" section, floor-space-config-form, floor-space-grid, floor-space-cell-dialog)
