...
웹 사이트의 주요 메뉴 구조와 각 섹션의 기능을 설명합니다.

### Dashboard

사이트 수, 랙 수, 장비 수, 테넌트 수, 전력 피드 수, 현재 입실 인원을 한눈에 확인. 최근 입출입 기록도 표시.

### Infrastructure

#### Sites

데이터센터 시설(건물) 관리. 지역(Region)·테넌트와 연결되며, 시설명·주소·위경도를 기록. 각 사이트 하위에 Location(서버실) → Rack 구조로 연결됨.

#### Regions

사이트들을 묶는 지리적 단위(예: 서울 수도권). Region에 여러 Site를 포함.

### Assets

#### Devices

서버, 네트워크 장비, 스토리지 등 개별 장비 관리. 랙 슬롯 배치, 상태(planned/active/decommissioned), 보증 만료일, 테넌트 태깅, 벌크 상태 변경 지원.

#### Tenants

데이터센터를 사용하는 조직(고객사/부서). 장비·랙·사이트에 테넌트를 지정해 소유권과 청구 단위를 구분.

#### Racks

전체 랙 목록. 사이트·위치별로 분류되며 개별 랙 클릭 시 U단위 엘리베이션(전면/후면) 뷰 표시. 드래그 앤 드롭으로 장비 배치 변경.

### Network

#### Cables

장비 간 케이블 연결(선번장) 관리. Cat5e/Cat6/광 등 타입별 분류, 양단 종단(Interface/FrontPort/RearPort) 지정, 케이블 추적(Trace) 지원.

#### Topology

네트워크 연결 토폴로지 시각화. 장비-인터페이스-케이블 관계를 그래프로 표시. 노드 드래그로 레이아웃 저장, 패치 패널 경유 경로 표시.

#### Power

전력 패널·피드 관리와 실시간 전력 소비 모니터링. SSE 기반 실시간 업데이트, 랙별 전력 그리드 뷰, 전력 용량 게이지 표시.

### Operations

#### Access

인력 입출입(체크인/체크아웃) 기록 및 장비 반출입(입고/출고/재배치/RMA) 관리. 승인 워크플로우 포함.

#### Reports

데이터 내보내기(Excel/XML) 및 CSV 일괄 가져오기. 정기 리포트 스케줄 설정(일/주/월). 지원 대상: 장비, 랙, 전력, 케이블, 사이트, 테넌트.

#### Alerts

전력 임계값·보증 만료·랙 용량 기반 알림 규칙 설정. 알림 히스토리 조회. Slack/이메일/인앱 알림 채널 설정.

### Admin

#### User Management

사용자 계정 관리 (관리자 전용). 역할: admin(모든 권한) / operator(읽기+쓰기) / viewer(읽기 전용) / tenant_viewer(담당 테넌트만).
